...
router.route("/reset-password/:resetToken").patch(auth.resetPasswordController);
...
router
  .route("/register")
  .post(rateLimiter, isLoginMiddleware, auth.signupController);

// login route using post method
router
  .route("/login")
  .post(rateLimiter, isLoginMiddleware, auth.loginController);

// logout route using post method
router.route("/logout").post(authMiddleware, auth.logoutController);

// register forgot password using post method
router.route("/forgot-password").post(auth.forgotPasswordController);
